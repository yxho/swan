...
private:
  ...
  Logger()
      : writer_(make_unique<ohxy::StdoutWriter>()),
        threadSync_(false),
        threadExit_(false),
        outputFull_(false),
    ...
    sinkThread_ = std::thread(&Logger::sinkThreadFunc, this);
  }
...
private:
  std::unique_ptr<ohxy::Writer> writer_;
...
inline void setLogLevel(LogLevel level) {
  Logger::singleton()->setLogLevel(level);
}

/// Get log level.
inline LogLevel getLogLevel() { return Logger::singleton()->getLogLevel(); }
...
inline void setLogFile(const char *file) {
  Logger::singleton()->setLogFile(file);
}

/// Set max log file size (MB).
inline void setMaxSize(size_t nMB) { Logger::singleton()->setMaxSize(nMB); }

/// Set max log file count.
inline void setMaxBackups(size_t count) {
  Logger::singleton()->setMaxBackups(count);
}

inline void setWriter(std::unique_ptr<ohxy::Writer> w) {
  Logger::singleton()->setWriter(std::move(w));
}

/// Back-end provides produce interface.
inline void produceLog(const char *data, size_t n) {
  Logger::singleton()->produce(data, n);
}

/// Move consumable position with increaing to ensure a complete log is consumed
/// each time.
inline void incConsumablePos(uint32_t n) {
  Logger::singleton()->incConsumable(n);
}
...
#define LOG(level, loc)                                                        \
  if (ohxy::getLogLevel() <= level)                                          \
  ohxy::LogLine(level, loc)

/// Create a logline with log level \p level , but without log localtion.
#define LOG_N_LOC(level) LOG(level, ohxy::LogLoc())

#define LOG_TRACE LOG_N_LOC(ohxy::LogLevel::TRACE)
#define LOG_DEBUG LOG_N_LOC(ohxy::LogLevel::DEBUG)
#define LOG_INFO LOG_N_LOC(ohxy::LogLevel::INFO)
#define LOG_WARN LOG_N_LOC(ohxy::LogLevel::WARN)
#define LOG_ERROR LOG_N_LOC(ohxy::LogLevel::ERROR)
#define LOG_FATAL LOG_N_LOC(ohxy::LogLevel::FATAL)

/// Create a logline with log level \p level and the log localtion.
#define LOG_LOC(level)                                                         \
  LOG(level, ohxy::LogLoc(__REL_FILE__, __FUNCTION__, __LINE__))

#define OHXY_LOG_TRACE LOG_LOC(ohxy::LogLevel::TRACE)
#define OHXY_LOG_DEBUG LOG_LOC(ohxy::LogLevel::DEBUG)
#define OHXY_LOG_INFO LOG_LOC(ohxy::LogLevel::INFO)
#define OHXY_LOG_WARN LOG_LOC(ohxy::LogLevel::WARN)
#define OHXY_LOG_ERROR LOG_LOC(ohxy::LogLevel::ERROR)
#define OHXY_LOG_FATAL LOG_LOC(ohxy::LogLevel::FATAL)